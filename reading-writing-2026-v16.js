(function(){
'use strict';
/* TOEFL Study Lab 2026 Reading/Writing structure patch v16
   Aligns practice UI with the post-2026-01-21 TOEFL iBT task families.
*/
...
const ACADEMIC_GROUPS=[
  {
    ...
    text:'Cities are often warmer than nearby rural areas, a pattern known as the urban heat island effect. One reason is that dark surfaces such as asphalt and rooftops absorb solar energy during the day and release heat slowly at night. Buildings can also reduce air circulation, while a relative lack of vegetation limits cooling through shade and evaporation. Researchers have found that planting trees, installing reflective roofs, and increasing access to green space can reduce local temperatures. These measures may also provide other benefits, including improved air quality and more comfortable outdoor spaces.',
    qs:[
      ...
      ['Why does the author mention asphalt and rooftops?',['To give examples of surfaces that store heat','To explain why buildings improve air circulation','To compare construction costs'],0],
      ['The word “limits” is closest in meaning to',['reduces','measures','creates'],0],
      ['Which benefit of green measures is mentioned in addition to cooling?',['Improved air quality','Lower university tuition','Faster public transportation'],0]
    ]
  },
  {
    title:'Academic Passage 2 — Memory and retrieval',
    text:'Learning is strengthened not only by studying information but also by retrieving it from memory. In many experiments, students who test themselves on material remember more later than students who spend the same amount of time rereading it. Retrieval appears to make a memory easier to access in the future, especially when learners must produce an answer rather than simply recognize it. Feedback is also important because it helps students correct errors before those errors become firmly established. For this reason, low-stakes quizzes can function as learning activities rather than merely as tools for measuring performance.',
    qs:[
      ...
      ['What can be inferred about low-stakes quizzes?',['They can support learning as well as assess it','They are useful only for grading','They should contain no feedback'],0]
    ]
  }
];

function esc(s){return String(s).replace(/[&<>\"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[m]));}
function mcq(id,p,c,a){return '<div class="q"><div><b>'+esc(p)+'</b></div>'+c.map((x,i)=>'<label class="option"><input type="radio" name="'+id+'" value="'+i+'"> '+esc(x)+'</label>').join('')+'<div class="actions"><button class="ghost ckmcq" data-name="'+id+'" data-a="'+a+'">檢查答案</button></div><div class="feedback"></div></div>';}
function ctest(){
...
function groupedReading(group,prefix,start){
  let h='<div class="q" style="background:#fff"><div class="small">'+esc(group.title)+'</div><div style="font-size:1.04rem;line-height:1.75;margin-top:5px">'+esc(group.text)+'</div></div>';
  group.qs.forEach((q,i)=>{h+=mcq(prefix+(start+i),q[0],q[1],q[2]);});
  return h;
}
function readingHTML(mock){
  let h='<h3>'+ (mock?'Mock Reading':'Reading｜20題訓練') +'</h3><div class="accent-note"><b>2026 新制 Reading</b><br><span class="small">Complete the Words → Read in Daily Life → Read an Academic Passage。本站日常訓練固定 20 題；正式考試 Reading 基準約 50 items／30 分鐘，並採 adaptive 設計。</span></div>';
  h+=ctest();
...
function enhanceBuildSentence(root){
  (root||document).querySelectorAll('input[type="text"][id]').forEach(input=>{
    if(!/^(ws|mws)\d+$/.test(input.id)||input.dataset.v16chunks==='1')return;
...
function installReading(){
  ...
  if(!isReading)return;
  if(c.dataset.rw2026v16==='1')return;
  c.dataset.rw2026v16='1';c.innerHTML=readingHTML(false);
...
function enhanceCurrent(){installReading();installWriting();const mock=document.getElementById('mock');if(mock)enhanceBuildSentence(mock);}
  ...
  window.mockPart=function(p){
    ...
    setTimeout(()=>{
      ...
      if(p==='mr'){
        ...
        const score=document.createElement('div');score.className='score';score.id='mr-v16-score';score.textContent='Mock Reading：尚未計分';m.appendChild(score);
